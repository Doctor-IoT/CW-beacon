...
void *MorseSender::operator new(size_t size) {
  return malloc(size);
}

void MorseSender::operator delete(void *ptr) {
  if (ptr) free(ptr);
}
...
void LEDMorseSender::setOn() {
  ...
  beacon.output_enable(SI5351_CLK0, 1);
}
...
// PWMMorseSender
void PWMMorseSender::setOn() {
  analogWrite(pin, brightness);
}

void PWMMorseSender::setOff() {
  analogWrite(pin, 0);
}
